...
class TestFirstAPI:
    ...
    def test_hello_call(self, name_):
        ...
        data = {'name': name_}    # де 'name' - є параметром даного методу(endpoint-а), а 'name_' - ім'я змінної, через яку пропихаються дані, які зберігаються в змінній 'names'
...
# для тесту з порожнім іменем формується строка з порожнім іменем ('Hello, '),
# а насправді нам приходить строка з дефолтним значенням someone, тобто нам приходить 'Hello, someone'
