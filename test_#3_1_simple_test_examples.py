...
# pytest - бібліотека, що дозволяє звичайну послідовність дій оформляти у тести і стандартизувати спосіб перевірки будь-чого.
...
# через ключ -k ми вказуємо конкретно той тест, який ми будемо запускати з файлу (якщо у файлі кілька тестів, а нам потрібно запустити лише конкретний тест)
...
# через знак мінус pytest пише те значення, яке ми очікували, а через знак плюс pytest пише те значення, яке ми отримали насправді
# другим параметром до assert може іти сам текст помилки
# назви усіх тестів мають починатись зі слова test, так само і назва класу у файлі з тестами має починатись на слово test
