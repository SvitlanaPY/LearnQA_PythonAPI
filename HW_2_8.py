...
# В этом случае на первый запрос API начинает выполнение задачи,
# а на последующие ЛИБО говорит, что задача еще не готова, ЛИБО выдает результат.
#
# Сегодня я предлагаю протестировать такой API-метод.
#
# Сам API-метод находится по следующему URL: https://playground.learnqa.ru/ajax/api/longtime_job
# Если мы вызываем его БЕЗ GET-параметра token, метод заводит/создает новую задачу,
# а в ответ выдает нам JSON со следующими полями:
# * seconds - количество секунд, через сколько задача будет выполнена и
...
# 1) создавал задачу
# 2) делал один запрос с token ДО того, как задача будет готова, убеждался в правильности поля status (т.е. status=задача еще не готова)
# 3) ждал нужное количество секунд с помощью функции time.sleep() - для этого надо сделать import time
# 4) делал бы один запрос c token ПОСЛЕ того, как задача готова, убеждался в правильности поля status(т.е. status=задача готова) и наличии поля result
#
...
# parsed_response = response.json()   # {dict: 2} {'token': 'QNyozN1oDMwACMy0SNw0iNyAjM', 'seconds': 16}
